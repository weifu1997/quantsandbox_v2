...
OUTPUT = Path("data/reports/pbindlow_narrow_oos_corrected.json")
STATIC_WINDOWS = [
...
def apply_filter(dataset: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    filtered = dataset.loc[(dataset["trend_20d"] == "downtrend") & (dataset["breadth_regime"] == "narrow_weakness")].copy()
...
def run_block(label: str, dataset: pd.DataFrame, coverage: dict) -> dict:
    ...
    validation = run_factor_validation(dataset, factor_col=factor_col, horizons=[HORIZON], groups=5, split_date=resolve_split_date(dataset))
    backtest = run_topn_backtest(dataset=dataset, factor_col=factor_col, top_n=TOP_N, rebalance_frequency=REBALANCE_FREQUENCY, weighting=WEIGHTING, benchmark=BENCHMARK, commission_bps=COMMISSION_BPS, slippage_bps=SLIPPAGE_BPS, horizon=HORIZON).payload
...
def run() -> Path:
    ...
    for label, obs_start, obs_end, test_start, test_end in ROLLING_WINDOWS:
        ds = build_dataset(tickers, test_start, test_end)
        filtered, coverage = apply_filter(ds)
    ...
    report = {
        "report_type": "pbindlow_narrow_oos_corrected",
        "generated_at": pd.Timestamp.now("UTC").isoformat(),
        "config": {"horizon": HORIZON, "rebalance_frequency": REBALANCE_FREQUENCY, "top_n": TOP_N},
