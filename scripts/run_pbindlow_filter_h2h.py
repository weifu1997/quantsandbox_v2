...
FILTERS = [
    ...
    ("trend_breadth_double_filter", {"regime_trend_60d": "downtrend", "regime_breadth_20d": "narrow_weakness"}),
]
...
def apply_filter(dataset: pd.DataFrame, condition: dict[str, str]) -> tuple[pd.DataFrame, dict]:
...
def run() -> Path:
    ...
    for window_label, start_date, end_date in WINDOWS:
        base_dataset, dataset_summary = build_dataset(tickers, start_date, end_date)
        regimes = build_market_regimes(base_dataset)
        merged = base_dataset.merge(regimes, on="date", how="left")
        for filter_label, condition in FILTERS:
            filtered, coverage = apply_filter(merged, condition)
            results.append(run_single(f"{filter_label}__{window_label}", filtered, coverage))

    report = {
        "report_type": "pb_industry_lowpb_filter_headtohead",
        ...
        "dataset_summary_last_run": dataset_summary,
        "results": results,
        "notes": [
            "Formal head-to-head comparison between downtrend_only and double_filter.",
            "Both are evaluated on the same expanded-1000 sample and same core parameters.",
            "Windows include full period, 2024 only, and 2025 only to assess cross-year consistency.",
        ],
    }

    out_path = Path("data/reports") / f"pbindlow_filter_h2h_{pd.Timestamp.now('UTC').strftime('%Y%m%dT%H%M%SZ')}.json"
