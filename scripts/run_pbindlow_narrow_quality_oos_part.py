...
BLOCKS = {
    "static__y2024": ("20240101", "20241231"),
    "static__y2025": ("20250101", "20251231"),
    "rolling_test__2024H1_to_2024H2": ("20240701", "20241231"),
    "rolling_test__2024H2_to_2025H1": ("20250101", "20250630"),
    "rolling_test__2025H1_to_2025H2": ("20250701", "20251231"),
}
...
def apply_filter(dataset: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
...
def run_block(block_label: str) -> Path:
    start_date, end_date = BLOCKS[block_label]
    ds = build_dataset(start_date, end_date)
    filtered, coverage = apply_filter(ds)
    factor_col = factor_column(FACTOR_NAME)
    validation = run_factor_validation(filtered, factor_col=factor_col, horizons=[HORIZON], groups=5, split_date=resolve_split_date(filtered))
    backtest = run_topn_backtest(dataset=filtered, factor_col=factor_col, top_n=TOP_N, rebalance_frequency=REBALANCE_FREQUENCY, weighting=WEIGHTING, benchmark=BENCHMARK, commission_bps=COMMISSION_BPS, slippage_bps=SLIPPAGE_BPS, horizon=HORIZON).payload
    out = {
        "report_type": "pbindlow_narrow_quality_oos_part",
    ...
    out_path = OUT_DIR / f"quality_oos_{block_label}.json"
...
def main() -> None:
    ...
    parser.add_argument("--block", required=True, choices=sorted(BLOCKS.keys()))
