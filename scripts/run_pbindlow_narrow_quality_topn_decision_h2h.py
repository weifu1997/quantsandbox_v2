...
def build_dataset(start_date: str, end_date: str) -> tuple[pd.DataFrame, dict]:
...
def summarize_window(label: str, start_date: str, end_date: str) -> dict:
    dataset, dataset_summary = build_dataset(start_date, end_date)
    validation = run_factor_validation(dataset, factor_col=factor_column(FACTOR_NAME), horizons=[HORIZON], groups=5, split_date=resolve_split_date(dataset))
    diag = validation["full_sample"][str(HORIZON)]["diagnostics"]["summary"]
    results = {}
    for top_n in TOP_NS:
        ...
        print(f"[{label}] done top_n={top_n}", flush=True)
    return {
        "window": {"label": label, "start_date": start_date, "end_date": end_date},
        "dataset_summary": dataset_summary,
        "coverage": {"rows": int(len(dataset)), "active_dates": int(dataset["date"].nunique())},
        "validation_summary": diag,
        "results": results,
    }


def main() -> None:
    full = summarize_window("full_2024_2026H1", "20240101", "20260514")
    review = summarize_window("review_2026H1", "20260101", "20260514")

    report = {
        "report_type": "pbindlow_narrow_quality_topn_decision_h2h",
        "generated_at": pd.Timestamp.now("UTC").isoformat(),
        "candidate": "pbindlow_downtrend_narrow_quality_v1",
        "comparison": {
            "current_registry_top_n": 10,
            "proposed_top_n": 20,
        },
        "windows": [full, review],
        "decision_hint": {
            "full_sample_better_top_n": 20 if full["results"]["20"]["sharpe"] > full["results"]["10"]["sharpe"] else 10,
            "review_window_better_top_n": 20 if review["results"]["20"]["sharpe"] > review["results"]["10"]["sharpe"] else 10,
        },
        "notes": [
            "Formal head-to-head to decide whether the official primary candidate should migrate from top_n=10 to top_n=20.",
            "Uses the same quality-refined value filter and compares both the longer full sample and the latest weakening review window.",
        ],
    }

    out_path = Path("data/reports") / f"pbindlow_narrow_quality_topn_decision_h2h_{pd.Timestamp.now('UTC').strftime('%Y%m%dT%H%M%SZ')}.json"
