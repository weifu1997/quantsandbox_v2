...
def run() -> Path:
    ...
    report = {
        "report_type": "pbindlow_narrow_quality_topn_robustness",
        ...
        "ranked": ranked,
        "notes": [
            "Focused robustness check for the new official value primary candidate.",
            "Use this to judge whether top_n=10 remains the best operating default before engineering promotion.",
        ],
    }
    out_path = Path("data/reports") / f"pbindlow_narrow_quality_topn_robustness_{pd.Timestamp.now('UTC').strftime('%Y%m%dT%H%M%SZ')}.json"
