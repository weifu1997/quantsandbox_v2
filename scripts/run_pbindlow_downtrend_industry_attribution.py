...
OUTPUT = Path("data/reports/pbindlow_downtrend_industry_attribution_20260517.json")
WINDOWS = [
...
def build_regime(ds: pd.DataFrame) -> pd.DataFrame:
...
def summarize_window(ds: pd.DataFrame, label: str) -> dict:
    merged = ds.merge(build_regime(ds), on="date", how="left")
    ...
    for dt, tickers in holdings.items():
        for t in tickers:
            ind = filtered.loc[filtered["ticker"] == t, "industry"].dropna()
            rows.append({"date": dt, "ticker": t, "industry": ind.iloc[0] if not ind.empty else None})
    hold_df = pd.DataFrame(rows)
    if hold_df.empty:
        return {"label": label, "industry_counts": {}, "top3_industry_share_mean": 0.0, "avg_industry_count_per_rebalance": 0.0}
    per_date = hold_df.groupby(["date","industry"]).size().reset_index(name='count')
    top3_share = []
    ind_counts = []
    for dt, g in per_date.groupby('date'):
        total = g['count'].sum()
    ...
    industry_counts = hold_df['industry'].value_counts().head(10).to_dict()
    return {
        "label": label,
...
def run() -> Path:
    ...
    out = {"report_type":"pbindlow_downtrend_industry_attribution","generated_at":pd.Timestamp.now('UTC').isoformat(),"results":results}
