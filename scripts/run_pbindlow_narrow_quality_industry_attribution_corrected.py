...
OUTPUT = Path("data/reports/pbindlow_narrow_quality_industry_attribution_corrected.json")
WINDOWS = [
...
def apply_filter(dataset: pd.DataFrame) -> pd.DataFrame:
    ...
    return dataset.loc[(dataset["trend_20d"] == "downtrend") & (dataset["breadth_regime"] == "narrow_weakness") & (roe > 0) & (pg > -0.2)].copy()
...
def run() -> Path:
    ...
    out = {"report_type": "pbindlow_narrow_quality_industry_attribution_corrected", "generated_at": pd.Timestamp.now("UTC").isoformat(), "results": results}
