...
def load_expanded_tickers(limit: int = SAMPLE_LIMIT) -> list[str]:
    ...
    if len(tickers) < limit:
        raise ValueError(f"not enough tickers for sample limit {limit}: only {len(tickers)}")
...
def run_variant(name: str, filtered: pd.DataFrame, coverage: dict) -> dict:
    ...
    return {'label': name, 'coverage': coverage, 'rows': int(len(filtered)), 'validation': validation, 'backtest': backtest}


def run() -> Path:
    tickers = load_expanded_tickers(SAMPLE_LIMIT)
    ds, summary = build_dataset(tickers)
    ds = ds.merge(build_market_regimes(ds), on='date', how='left')
    results = []
    for label, cond in VARIANTS:
        filtered, coverage = apply_filter(ds, cond)
        results.append(run_variant(label, filtered, coverage))

    out = {
        'report_type': 'pbindlow_large_sample_review_batch2',
        ...
        'results': results,
        'notes': [
            'Task 20 batch 2: larger sample review on 2000 main-board names with full-period default vs enhanced versions.',
            'Purpose: test whether default/enhanced role split remains stable under larger sample expansion.',
        ],
    }
    out_path = Path('data/reports') / f"pbindlow_batch2_large_{pd.Timestamp.now('UTC').strftime('%Y%m%dT%H%M%SZ')}.json"
