...
export function useDeployability(
  deployability: MaybeRefOrGetter<DeployabilitySummary | null | undefined>,
): UseDeployabilityReturn {
  const growth = computed<DeployabilityItem | null>(() => unref(deployability)?.growth ?? null)
  const valuePrimary = computed<DeployabilityItem | null>(() => unref(deployability)?.value_primary ?? null)
  const valueBaselineReference = computed<DeployabilityItem | null>(
    () => unref(deployability)?.value_baseline_reference ?? null,
  )
