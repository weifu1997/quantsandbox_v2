...
<td>{{ fmtCurrency(row.realized_pnl_total) }}</td>
<td>{{ fmtCurrency(row.unrealized_pnl) }}</td>
<td :class="row.total_pnl >= 0 ? 'success-text' : 'danger-text'">{{ fmtCurrency(row.total_pnl) }}</td>
<td>{{ fmtNumber(row.latest_shares_held) }}</td>
<td>{{ fmtNumber(row.latest_avg_cost) }}</td>
...
<td>{{ row.realized_pnl === '' ? '—' : fmtCurrency(Number(row.realized_pnl)) }}</td>
</tr>
</tbody>
</table>
</div>
...
<td>{{ fmtCurrency(row.realized_pnl) }}</td>
</tr>
</tbody>
</table>
</div>
